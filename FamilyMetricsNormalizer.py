...
def compute_optical_similarity(
    fm1: FontMeasures, fm2: FontMeasures, threshold: float
) -> bool:
    ...
    if (
        ...
        if span1 and span2 and span1 > 0 and span2 > 0:
            ...
            # If span differs by more than 50%, they're structurally incompatible
            if span_ratio > 1.5:
                return False
...
def detect_decorative_outlier(
    fm: FontMeasures, core_cluster: List[FontMeasures], threshold: float
) -> bool:
    ...
    if fm.max_y is not None and fm.min_y is not None and fm.upm > 0:
        ...
        if cluster_spans:
            ...
            if avg_cluster_span > 0:
                ...
                # If span differs by more than 50%, it's structurally different (not decorative)
                if span_ratio > 1.5:
                    return False

    # Check if core metrics match any font in cluster
    matches_core = any(
        compute_optical_similarity(fm, core_fm, threshold) for core_fm in core_cluster
    )
...
def detect_optical_clusters(
    measures: List[FontMeasures], threshold: float
    ...
    for i in range(n):
        for j in range(i + 1, n):
            if compute_optical_similarity(measures[i], measures[j], threshold):
    ...
    for fm in singletons:
        if detect_decorative_outlier(fm, main_cluster, threshold):
...
def compute_family_normalized_ascender(
    ...
    Uses fixed percentage of UPM for headroom (default 25%), then adjusts upward
    if actual ascenders exceed the planned headroom.
    """
    cap_ratios: List[float] = []
    ...
    # Fixed headroom as percentage of UPM (25% default = 0.25)
    base_headroom = config.headroom_ratio
    planned_ascender = max_cap_ratio + base_headroom

    # Check if any font's ascenders exceed cap + half the planned headroom
    if ascender_ratios:
        max_ascender_ratio = max(ascender_ratios)
        # Minimum threshold: max(half headroom, 2% of UPM)
        threshold = max(base_headroom / 2.0, 0.02)

        # If ascender exceeds cap + threshold, increase headroom by difference
        if max_ascender_ratio > (max_cap_ratio + threshold):
            extra_headroom = max_ascender_ratio - max_cap_ratio - threshold
            planned_ascender = max_cap_ratio + threshold + extra_headroom

    return planned_ascender
    ...
    for fam, group in families.items():
        ...
        if len(group) > 1:
            clusters, decorative_outliers = detect_optical_clusters(
                group, config.optical_threshold
            )
            ...
            if decorative_outliers and main_cluster:
